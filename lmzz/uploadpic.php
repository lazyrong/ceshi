...
$img_path='./imgs/boy/'.$_REQUEST['num'].'.jpg';
$pic_type = 'jpg';
$img=getImgType($img_path);
if (!$_COOKIE['v_o_t_']) {
	header('Location: ./login.php?id=q&app=adj2');
	exit;
}
if ($pic_type == 'jpg') {
	$img_filename = SAE_TMP_PATH . '/' . cookie('v_o_t_'). '.jpg';
	imagejpeg ($img, $img_filename );
} else {
	$img_filename = SAE_TMP_PATH . '/' . cookie('v_o_t_'). '.png';
