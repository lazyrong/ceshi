...
<div class="head_clear"></div>
 ...
 <div class="cont_box">
    <div id="all_apps">
      ...
      <?php 
      $idx = 0;
      $app_list = array_reverse($app_list);
      foreach ($app_list as $appname => $hot_app) {
      		$idx++;
      	if ($hot_app['logo']) {
      ?>
      <li> <a target="_blank" href="./<?=$appname?>" title="<?=$hot_app['title']?>，<?=$hot_app['num']?>人在玩~"><img src="<?=$hot_app['logo']?>" /></a> </li>
      <?php 
		} else {
	?>
      <li> <a target="_blank" href="./<?=$appname?>" title="<?=$hot_app['title']?>，<?=$hot_app['num']?>人在玩~"><img src="http://app.qpic.cn/mblogpic/4a2af0ed5f96e16ff036/460" /></a> </li>
	<?php 
      }
      }
      $leftitem = $idx % 4;
