	...
	if ($_COOKIE ['v_type'] == 't') {
		...
		if ($imgurl) {
			...
			if ($pagram [img]) {
				...
				$uploadfile = $stor->upload ( 'vceshi7', $destFileName, $img_filename );
				if ($uploadfile) {
					$cdnurl = $stor->getCDNUrl ( 'vceshi7', $destFileName );
					$uploadfile = $cdnurl ? $cdnurl : $stor->getUrl ( 'vceshi7', $destFileName );
